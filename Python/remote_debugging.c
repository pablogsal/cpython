 ...
 *
 * This file implements the sender side of the remote debugging protocol:
 * sys.remote_exec() writes a script path into a target process's thread
 * state and sets the eval_breaker flag so the target picks it up.
 *
 * Linux only - uses process_vm_readv/process_vm_writev.
 */
...
#ifdef __linux__
#define Py_REMOTE_DEBUG
#define Py_SUPPORTS_REMOTE_DEBUG
#endif
...
send_exec_to_proc_handle(proc_handle_t *handle, int tid, const char *debugger_script_path)
{
    ...
    /* Check if remote debugging is enabled in the remote process */
    ...
    /* Find the target thread state */
    uintptr_t thread_state_addr;
    unsigned long this_tid = 0;

    if (tid != 0) {
        /* Walk the thread list to find the specific thread */
        if (0 != read_memory(
                handle,
        ...
        /* Use the main thread - read threads.head as the first thread */
        if (0 != read_memory(
                handle,
                interpreter_state_addr + (uintptr_t)debug_offsets.interpreter_state.threads_head,
                sizeof(void*),
    ...
    /* Write the script path to the thread's remote_debugger_support struct */
    ...
    /*
     * Set the eval_breaker flag on the interpreter to interrupt the eval loop.
     * In 3.12, eval_breaker is in interp->ceval (per-interpreter, not per-thread).
     * debug_offsets.debugger_support.eval_breaker stores the offset from
     * the interpreter state to the eval_breaker field.
     */
    int eval_breaker_val;
    uintptr_t eval_breaker_addr = interpreter_state_addr +
        (uintptr_t)debug_offsets.debugger_support.eval_breaker;

    if (0 != read_memory(
            handle,
            eval_breaker_addr,
            sizeof(eval_breaker_val),
            &eval_breaker_val))
    {
        return -1;
    }
