...
static inline void
_set_debug_exception_cause(PyObject *exception, const char *string)
{
    if (!PyErr_ExceptionMatches(PyExc_PermissionError)) {
        if (!PyErr_Occurred()) {
            PyErr_SetString(exception, string);
        } else {
            _PyErr_FormatFromCause(exception, "%s", string);
        }
    }
}
...
return_section_address64(
    ...
    PyErr_Format(PyExc_RuntimeError,
        "Section '%s' not found in 64-bit Mach-O binary at base address 0x%lx",
        section, base);
    return 0;
}
...
return_section_address32(
    ...
    PyErr_Format(PyExc_RuntimeError,
        "Section '%s' not found in 32-bit Mach-O binary at base address 0x%lx",
        section, base);
    return 0;
}
...
search_map_for_section(proc_handle_t *handle, const char* secname, const char* substr) {
    ...
    PyErr_Format(PyExc_RuntimeError,
        "Section '%s' not found in any binary matching '%s' for PID %d (%d VM regions examined)",
        secname, substr, handle->pid, region_count);
    return 0;
}
        ...
        const char *elf_file)
{
    ...
    if (section == NULL) {
        PyErr_Format(PyExc_RuntimeError,
            "Section '%s' not found in ELF file '%s' (%u sections examined)",
            secname, elf_file, elf_header->e_shnum);
        goto exit;
    }
...
search_linux_map_for_section(proc_handle_t *handle, const char* secname, const char* substr)
{
    ...
    if (retval == 0) {
        PyErr_Format(PyExc_RuntimeError,
            "Section '%s' not found in any ELF file matching '%s' for "
            "PID %d (%d map entries processed, %d matches found)",
            secname, substr, handle->pid, lines_processed, matches_found);
    }
...
static void* analyze_pe(const wchar_t* mod_path, BYTE* remote_base, const char* secname) {
    ...
    if (runtime_addr == NULL) {
        PyErr_Format(PyExc_RuntimeError,
            "Section '%s' not found in PE file (%d sections examined, remote base: 0x%p)",
            secname, pNTHeaders->FileHeader.NumberOfSections, remote_base);
    }
...
search_windows_map_for_section(proc_handle_t* handle, const char* secname, const wchar_t* substr) {
    ...
    for (BOOL hasModule = Module32FirstW(hProcSnap, &moduleEntry); hasModule; hasModule = Module32NextW(hProcSnap, &moduleEntry)) {
        ...
        if (wcsstr(moduleEntry.szModule, substr)) {
            ...
            if (runtime_addr != NULL) {
                ...
                _set_debug_exception_cause(PyExc_RuntimeError, "PE analysis failed for matched module");
            }
        }
    }
    ...
    if (runtime_addr == NULL) {
        PyErr_Format(PyExc_RuntimeError,
            "Section '%s' not found in any module matching pattern for "
            "PID %d (%d modules examined, %d matches found)",
            secname, handle->pid, modules_examined, matches_found);
    }
