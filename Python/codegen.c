...
codegen_formatted_value(compiler *c, expr_ty e)
{
    ...
    /* PEP 813: !p conversion with callable format_spec */
    if (conversion == 'p' && e->v.FormattedValue.format_spec) {
        /* f"{value!p:callable}" - call callable(value).
         * The format_spec holds the callable expression.
         * Stack layout for CALL: callable | NULL | arg */
